...
using LIGHT = MVision.USB_HID.PLC_C2S10.LIGHT;
using System.IO;
...
namespace MVision
{
    ...
    //}
        public  class _DLS{
        public enum HowMany { NO, CAM1, CAM1_CAM2 }
        [Serializable()]
        public class DT
        {
            // Camera Getings
            ...
            public decimal [] GEIN = new decimal[2];
            ...
            static public HowMany HowManyCam = HowMany.NO;  /*** No=0;  Master = 1/ Master+Slave = 2 **/
    ...
    public partial class DLS
    {
       ...
       public static _DLS.HowMany DLS_HowManyCAMERAS;

    public DLS(_DLS.HowMany howMany)
    {
            DLS_HowManyCAMERAS = howMany;
     ...
     if(( howMany == _DLS.HowMany.CAM1) ||(howMany == _DLS.HowMany.CAM1_CAM2)) 
      {
        ...
        if ( !SAV.DT.DALSA.SetingsCameraStart)
        {
            ...
            if (acConfigDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                if (acConfigDlg.ClousedDialogOK() == true)
                {
                    //acConfigDlg.Show();
        ...
        acConfigDlg.Close();
            ...
            if ((howMany == _DLS.HowMany.CAM1_CAM2))
            {
                ...
                {        
                    ...
                    if (acConfigDlg2.ShowDialog() == DialogResult.OK)
                        if (acConfigDlg2.ClousedDialogOK() == true)
                        {
    ...
    public bool InstCOM_Setings(int ID_Cam)
    {
        ...
        if (ID_Cam != ResourceIndex[ID_Cam]) { return false; }
            ...
            try { 
       location = new SapLocation(ServerName[ID_Cam], 0); //"CameraLink_1" //0
            }
            catch { return false; }
        ...
        public bool SetGain(double gainValue, int ID_Cam)
        {
            ...
            if (((DLS_HowManyCAMERAS == _DLS.HowMany.CAM1) && (ID_Cam != Slave)) || (DLS_HowManyCAMERAS == _DLS.HowMany.CAM1_CAM2)) { 
    ...
    static IntPtr DataM = new IntPtr(); // image pointer
       ...
       public static int[] CameraSinc = new int[2] { 0, 0 };
        ...
        static public void CheckForLastFrameMast(int number, bool trash)
    {
            ...
            else
            {
                ...
                //                         Emgu.CV.CvEnum.DepthType.Cv8U, 3, // 1-GRY | 3- RGB     //Emgu.CV.CvEnum.DepthType.Cv8U  
                //                         DataM,
                //                         DalsaVal.m_Buffers[Master].Width);



                // Створення Mat з правильним кроком пікселя
                Emgu.CV.Mat imOriginalBGR = new Emgu.CV.Mat(DalsaVal.m_Buffers[Master].Height,
                                               DalsaVal.m_Buffers[Master].Width,
                                               Emgu.CV.CvEnum.DepthType.Cv8U, 4,
                                               DataM,
                                               DalsaVal.m_Buffers[Master].Width * 4);



                //------------ Зміна формату зображення з grey на BGR
                // Mat imBGR = new Mat();
                // CvInvoke.CvtColor(imOriginal, imBGR, ColorConversion.Gray2Bgr);
                //------------------------------------------------

                if (START)
                {
                    IProducerConsumerCollection<Image<Bgr, byte>> CollecTempM = FlowCamera.BoxM;
                    CollecTempM.TryAdd(imOriginalBGR.ToImage<Bgr, byte>());
        ...
        static public void CheckForLastFrameSlav(int number, bool trash)
    {
        ...
        else
        {
            ...
            DalsaVal.m_Buffers[Slave].GetAddress(out DataS);
                ...
                // Створення Mat з правильним кроком пікселя
                Emgu.CV.Mat imOriginalBGR = new Emgu.CV.Mat(DalsaVal.m_Buffers[Slave].Height,
                                               DalsaVal.m_Buffers[Slave].Width,
                                               Emgu.CV.CvEnum.DepthType.Cv8U, 4,
                                               DataS,
                                               DalsaVal.m_Buffers[Slave].Width * 4);
                ...
                CollecTempS.TryAdd(imOriginalBGR.ToImage<Bgr, byte>());
    ...
    public void Acq_Dark_Click(int ID_Cam)
    {
        ...
        if (ID_Cam == Master) { SetGain((double)SAV.DT.DALSA.ACQGEIN1, Master); }
        else
        {
            SetGain((double)SAV.DT.DALSA.ACQGEIN2, Slave);
        }
    ...
    public Bitmap Acq_Dark_Simply(int ID_Cam, string PashSV)
    {
        ...
        if (ID_Cam == Master) { SetGain((double)SAV.DT.DALSA.ACQGEIN1_Black, Master); }
        else { SetGain((double)SAV.DT.DALSA.ACQGEIN2_Black, Slave); }

        System.Threading.Thread.Sleep(500);
        LIGHT.ON();
        DLS.ImgSnapSet = true;
        DLS.StartCAMERA(SAV.DT.DALSA.ID_CAM);
        System.Threading.Thread.Sleep(500);



        DLS.StopCAMERA(SAV.DT.DALSA.ID_CAM);
        ...
        if (DalsaVal.m_Xfer[ID_Cam] != null && DalsaVal.m_Xfer[ID_Cam].Initialized)
        {
            ...
            string Pash = Path.Combine(PashSV, "ImgAcq_Dark" + SAV.DT.DALSA.ID_CAM.ToString() + ".bmp"); //створити шлях до IMG
    ...
    {
        if (ID_Cam == Master) { SetGain((double)SAV.DT.DALSA.ACQGEIN1, Master); }
        else { SetGain((double)SAV.DT.DALSA.ACQGEIN2, Slave); }
        ...
        string Pash = Path.Combine(PashSV, "ImgAcq_" + SAV.DT.DALSA.ID_CAM.ToString() + ".bmp"); //створити шлях до IMG
        rotatedImage.Save(Pash);
